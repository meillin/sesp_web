dhtmlXGridObject.prototype.toPDF=function(url,mode,header,footer,rows){
	...
	//win.document.write('<form id="myform" method="post" action="'+url+'" accept-charset="utf-8"><input type="hidden" name="grid_xml" id="grid_xml"/> </form>');
	...
	var htmls = "<html><body>";	
	htmls = htmls +'<form id="myform" method="post" action="'+url+'" accept-charset="utf-8"><input type="hidden" name="grid_xml" id="grid_xml"/><input type="hidden" name="filename" id="filename"/> </form></body></html>';
	//document.getElementById("myIFrm").innerHTML= htmls;
	...
	//document.getElementById("myIFrm").document.getElementById("grid_xml").value = xml_top(mode).replace("\u2013", "-") + xml_body() + xml_end();
	//document.getElementById("myIFrm").document.getElementById("myform").submit();
	...
	ifrDoc.getElementById('filename').value=document.getElementById('cgi1date').value+"-"+document.getElementById('cgi1loc').value+"-"+document.getElementById('cgi1sloc').value;		
		
	theForm.submit();
	//downloadExcel(xml_top(mode).replace("\u2013", "-") + xml_body() + xml_end());
	//win.close();
	grid = null;
};
